...
public class Startup {
    ...
    private static CalAmpSIWrapper wrapData(byte[] calAmpDataBytes,
            List<String> initialPathPlan) {
        UUID siId = UUID.randomUUID();
        CalAmpSIWrapper payload = new CalAmpSIWrapper(siId, calAmpDataBytes,
                initialPathPlan);
        return payload;
    }

    private static String getDefaultBeanName(
    ...
    private static void runTest(ApplicationContext context)
        ...
        CalAmpSIRouteAndProcessService service = context.getBean(serviceName,
                CalAmpSIRouteAndProcessService.class);
         ...
         */
        while (!doStop) {
            String simCalAmpDataString = RandomStringUtils
                    .randomAlphanumeric(dataByteCount);
            byte[] calAmpDataBytes = simCalAmpDataString.getBytes(Charset
                    .forName("UTF-8"));
            CalAmpSIWrapper payload = wrapData(calAmpDataBytes, initialPathPlan);
            System.out.println("Service Request: " + payload);
            Future<CalAmpSIWrapper> asynchReply = service
                    .processMessage(payload);
            // The next line synchronizes. If no reply is necessary simply do
            // not execute .get().
            // If synchronization should happen in batches then collect the
            // futures and synchronize as desired.
    ...
    private static void timingTest(ApplicationContext context)
            throws InterruptedException, ExecutionException {
        boolean doStop = false;
        ...
        List<String> stageLabels = Arrays.asList("A", "B", "C", "D", "E", "F",
                "G", "H", "I");
        for (String label : stageLabels) {
            Transforms.registerStage(new GaussianSleeper(label, false, mu,
                    sigma));
        }
        Transforms.registerStage(new GaussianSleeper(stageLabels
                .get(stageLabels.size() - 1), true, mu, sigma));
